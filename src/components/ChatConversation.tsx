...
const ChatConversation = () => {
  ...
  const data: Array<{ time: number; content: any }> = [
    ...
    { time: 2500, content: 'If we\'re starting a new project together, we\'ll build it from the ground up to be insanely beautiful, easy to use, reliable, and of course blazingly fast 🔥' },
    { time: 2000, content: 'If we\'re working on an existing project, we\'ll increase its value with every iteration.' },
    { time: 2000, content: <>What do you say we get to work? <a href="mailto:kollin.murphy@gmail.com" target='_blank' class='underline hover:text-[#D8EDFE]'>Contact me</a> and let's get started!</> },
  ]
  ...
  const handleNextStep = () => {
    ...
    const i = index()
    if (i < data.length - 1) {
      const randomTime = Math.floor(Math.random() * 700 - 350)
